...
public class TeleOpRed extends LinearOpMode {
    ...
    int greencount = 0;
    double Time = 0;
    public int towerX = RED_TOWER_X;
    public int towerY = RED_TOWER_Y;
    ...
    private final Pose START_POSE = RED_START_POSE;
...
//        turret.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        parcare = () -> follower.pathBuilder()
                .addPath(new Path(new BezierLine(follower::getPose, RED_PARK_POSE)))
                .setHeadingInterpolation(HeadingInterpolator.linearFromPoint(follower::getHeading, Math.toRadians(RED_PARK_HEADING_DEG), PARK_HEADING_BLEND))
                .build();
...
//Tureta
            if (gamepad1.left_bumper){
                follower.setPose(RED_TURRET_OVERRIDE_POSE);
            }
            switch (stateTureta){
                case 0:
                    if (resumeFromLast){
...
//                    if (throwTimer.milliseconds() > 1200) {
                ...
                // ----------------- A DOUA BILA -----------------
                case 3:
                    if (throwTimer.milliseconds() > THROW_POSE_CHANGE_MS) {
                        if (!colectareSelectiva) {
                            indexer.OuttakePose2();
                            throwTimer.reset();
                            stateThrow = 4;
...
//Collect
            switch (stateCollect) {
                ...
                case 3: // PickPose3
                    if (collecttimer.milliseconds() > COLLECT_DELAY_BEFORE_TAKEOUT_MS) {
                        collecttimer.reset();
                        TakeOUT = true;
                        stateCollect = 4;
