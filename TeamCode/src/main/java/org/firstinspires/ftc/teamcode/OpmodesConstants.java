...
public class OpmodesConstants {
    ...
    public static int RED_TOWER_X = 129;
    /** Red alliance tower Y coordinate (field inches). Used for turret angle calculation. */
    public static int RED_TOWER_Y = 130;

    /** Blue alliance tower X coordinate (field inches). Used for turret angle calculation. */
    public static int BLUE_TOWER_X = 14;
    /** Blue alliance tower Y coordinate (field inches). Used for turret angle calculation. */
    public static int BLUE_TOWER_Y = 138;

    /** BNG Blue variant tower X coordinate (field inches). */
    public static int BNG_BLUE_TOWER_X = 12;
    /** BNG Blue variant tower Y coordinate (field inches). */
    public static int BNG_BLUE_TOWER_Y = 133;

    public static int APRILTAG_BLUE_TOWER_X = 16;
    public static int APRILTAG_BLUE_TOWER_Y = 130;
    public static int APRILTAG_RED_TOWER_X = 128;
    public static int APRILTAG_RED_TOWER_Y = 130;

    /**
     * AprilTag field positions (x, y inches) for pose fallback when Limelight botpose
     * returns zeros (no field map). Tower tags from game layout; backstage may need tuning.
     */
    public static double[][] APRILTAG_FIELD_XY = {
            {APRILTAG_BLUE_TOWER_X, APRILTAG_BLUE_TOWER_Y},   // 20
            {APRILTAG_RED_TOWER_X, APRILTAG_RED_TOWER_Y},     // 24
    };

    /** Limelight camera offset: forward (in) from robot center. Used for manual pose fallback. */
    public static double LIMELIGHT_CAM_OFFSET_FORWARD_IN = 0.0;
    /** Limelight camera offset: lateral (in), + left. Used for manual pose fallback. */
    public static double LIMELIGHT_CAM_OFFSET_LATERAL_IN = 0.0;
     ...
     */
    public static double TURRET_TICKS_PER_DEGREE = 6.071111;
     ...
     */
    public static double TURRET_TICKS_PER_DEGREE_LIMELIGHT = 6.533;
     ...
     */
    public static double LAUNCHER_CPR = 18.666;
    ...
    public static double LAUNCHER_DEFAULT_POWER = 0.9;

    /** Maximum allowed launcher RPM. Requests above this are clamped for safety. */
    public static int LAUNCHER_MAX_RPM = 12000;
    ...
    public static double INTAKE_POWER_KEEP_INSIDE = -0.7;
    ...
    public static double ASCENT_GRIP_POSITION = 0;
    /** Climber grip servo position when bar is released (open). */
    public static double ASCENT_RELEASE_POSITION = 0.4;

    /** Ascent lift power multiplier for descending (D) motors. */
    public static double ASCENT_POWER_D = 1.0;
    /** Ascent lift power multiplier for ascending (S) motors. */
    public static double ASCENT_POWER_S = 1.0;
    ...
    public static double PATH_MAX_POWER = 1.0;
    /** Mecanum X velocity (inches/s) - forward/back. */
    public static double PATH_X_VELOCITY = 73.908;
    /** Mecanum Y velocity (inches/s) - strafe. */
    public static double PATH_Y_VELOCITY = 58.436;

    /** Pinpoint forward pod Y offset (inches). */
    public static double PATH_FORWARD_POD_Y = 6.3;
    /** Pinpoint strafe pod X offset (inches). */
    public static double PATH_STRAFE_POD_X = -3.15;

    /** Robot mass (kg) for path follower feedforward. */
    public static double PATH_MASS = 9.52;
    /** Forward zero-power acceleration (in/s²). */
    public static double PATH_FORWARD_ZERO_POWER_ACCEL = -35.288;
    /** Lateral zero-power acceleration (in/s²). */
    public static double PATH_LATERAL_ZERO_POWER_ACCEL = -66.111;

    /** Translational PIDF kP – position error gain. */
    public static double PATH_TRANSLATIONAL_KP = 0.2;
    /** Translational PIDF kI. */
    public static double PATH_TRANSLATIONAL_KI = 0;
    /** Translational PIDF kD. */
    public static double PATH_TRANSLATIONAL_KD = 0.02;
    /** Translational PIDF kF – feedforward. */
    public static double PATH_TRANSLATIONAL_KF = 0.027;

    /** Heading PIDF kP. */
    public static double PATH_HEADING_KP = 1;
    /** Heading PIDF kI. */
    public static double PATH_HEADING_KI = 0;
    /** Heading PIDF kD. */
    public static double PATH_HEADING_KD = 0;
    /** Heading PIDF kF. */
    public static double PATH_HEADING_KF = 0.01;

    /** Drive (Filtered) PIDF kP. */
    public static double PATH_DRIVE_KP = 0.025;
    /** Drive PIDF kI. */
    public static double PATH_DRIVE_KI = 0;
    /** Drive PIDF kD. */
    public static double PATH_DRIVE_KD = 0.00001;
    /** Drive PIDF kV – velocity feedforward. */
    public static double PATH_DRIVE_KV = 0.6;
    /** Drive PIDF kA – acceleration feedforward. */
    public static double PATH_DRIVE_KA = 0.01;

    /** Path constraints: max velocity. */
    public static double PATH_CONSTRAINT_MAX_VELOCITY = 0.99;
    /** Path constraints: max acceleration. */
    public static double PATH_CONSTRAINT_MAX_ACCELERATION = 100;
    /** Path constraints: max angular velocity. */
    public static double PATH_CONSTRAINT_MAX_ANGULAR_VELOCITY = 10;
    /** Path constraints: max angular acceleration. */
    public static double PATH_CONSTRAINT_MAX_ANGULAR_ACCELERATION = 1;

    /** Launcher motor power when "charging" (reverse spin-up before shot). */
    public static double LAUNCHER_CHARGE_POWER = -0.8;
    ...
    public static Pose RED_START_POSE = new Pose(86, 117, Math.toRadians(0));
    /** Red alliance pose override when left bumper held. Positions robot for turret aim. */
    public static Pose RED_TURRET_OVERRIDE_POSE = new Pose(105, 140, Math.toRadians(0));
    /** Red alliance park destination (x, y) when cross button pressed. */
    public static Pose RED_PARK_POSE = new Pose(33, 33.4);
    /** Red alliance park heading in degrees. Robot faces this angle when parked. */
    public static int RED_PARK_HEADING_DEG = 45;

    /** Blue alliance starting pose: x, y (inches), heading (radians). */
    public static Pose BLUE_START_POSE = new Pose(57, 113, Math.toRadians(180));
    /** Blue alliance pose override when left bumper held. */
    public static Pose BLUE_TURRET_OVERRIDE_POSE = new Pose(39, 140, Math.toRadians(180));
    /** Blue alliance park destination (x, y). */
    public static Pose BLUE_PARK_POSE = new Pose(111, 29.5);
    /** Blue alliance park heading in degrees. */
    public static int BLUE_PARK_HEADING_DEG = 135;

    /** BNG Blue variant starting pose. */
    public static Pose BNG_BLUE_START_POSE = new Pose(40, 12, Math.toRadians(180));
    /**
     * Park heading interpolation blend factor (0–1).
     * Controls how quickly heading transitions toward park angle during path.
     */
    public static double PARK_HEADING_BLEND = 0.8;
    ...
    public static int AUTO_COLLECT_DISTANCE_MM_TIGHT = 60;
    /** Faster collect delay (ms) from PickPose1 to PickPose2 in some autos. */
    public static int AUTO_COLLECT_DELAY_POSE1_TO_2_MS_FAST = 500;
    /** Timeout (s) while driving to collect before giving up. */
    public static double AUTO_COLLECT_TIMEOUT_DRIVING_SEC = 1.5;
    /** Delay (s) before reading AprilTag to allow camera to stabilize. */
    public static double AUTO_TAG_DETECTION_DELAY_SEC = 0.05;
    /** Delay (s) before starting score path in BNG combo. */
    public static double AUTO_BNG_SCORE_DELAY_SEC = 0.5;

    /** Auto throw: extended RPM wait (ms) before first push. Some autos use looser tolerance. */
    public static int AUTO_THROW_RPM_WAIT_MS_EXTENDED = 700;
    /** Auto throw: mid RPM wait (ms) before push. */
    public static int AUTO_THROW_RPM_WAIT_MS_MID = 600;
    /** Auto throw: alternate RPM wait (ms) for subsequent balls. */
    public static int AUTO_THROW_RPM_WAIT_MS_ALT = 500;
    /** Auto throw: looser RPM tolerance (high) for "at speed" check. */
    public static int AUTO_THROW_RPM_TOLERANCE_HIGH = 150;
    /** Auto throw: looser RPM tolerance (low) for "at speed" check. */
    public static int AUTO_THROW_RPM_TOLERANCE_LOW = 50;

    /** Auto outtake angle for tower-style autos (close shot). */
    public static double AUTO_OUTTAKE_ANGLE_TOWER = 0.53;
    /** Auto outtake angle for mid-range tower shots. */
    public static double AUTO_OUTTAKE_ANGLE_MID = 0.72;
    /** Auto outtake angle default (used when no specific angle). */
    public static double AUTO_OUTTAKE_ANGLE_DEFAULT = 0.76;
    /** Auto outtake angle for triangle/small triangle autos (far shot). */
    public static double AUTO_OUTTAKE_ANGLE_TRIANGLE = 0.85;

    /** Default auto launcher RPM. */
    public static int AUTO_LAUNCHER_RPM_DEFAULT = 3500;
    /** Auto launcher RPM for triangle/small triangle paths. */
    public static int AUTO_LAUNCHER_RPM_TRIANGLE = 4900;
    /** Auto launcher RPM for BNG combo (first shot). */
    public static int AUTO_LAUNCHER_RPM_BNG = 5300;
    /** Auto launcher RPM for BNG combo (subsequent shots). */
    public static int AUTO_LAUNCHER_RPM_BNG_SECOND = 3600;
    /** Auto launcher RPM for far shot (e.g. last pickup). */
    public static int AUTO_LAUNCHER_RPM_FAR = 2800;
    /** Auto launcher RPM for close shot (e.g. Blue tower last pickup). */
    public static int AUTO_LAUNCHER_RPM_CLOSE_SHOT = 3000;

    /** Auto path follow speed (0–1) for slow segments. */
    public static double AUTO_PATH_SPEED_SLOW = 0.6;
    /** Auto path follow speed (0–1) for mid segments. */
    public static double AUTO_PATH_SPEED_MID = 0.7;
    /** Auto path follow speed (0–1) for fast segments. */
    public static double AUTO_PATH_SPEED_FAST = 0.8;
